...
test_that("`plot.dw_crosstab()` no percents", {
  skip_if_not_installed("datawizard")
  x <- datawizard::data_tabulate(iris_c$Sepal.Width_c, by = iris_c$Species)
  p <- plot(x)
...
test_that("`plot.dw_crosstab()` no percents", {
  skip_if_not_installed("datawizard")
  x <- datawizard::data_tabulate(
    iris_c$Sepal.Width_c,
    by = iris_c$Species,
    proportions = "row"
  )
  p <- plot(x)
...
test_that("`plot.dw_crosstab()` colum percents", {
  skip_if_not_installed("datawizard")
  x <- datawizard::data_tabulate(
    iris_c$Sepal.Width_c,
    by = iris_c$Species,
...
test_that("`plot.dw_crosstab()` all percents", {
  skip_if_not_installed("datawizard")
  x <- datawizard::data_tabulate(
    iris_c$Sepal.Width_c,
    by = iris_c$Species,
...
test_that("`plot.dw_groupmeans()` with no dots arguments works", {
  skip_if_not_installed("datawizard")
  data(iris)
  x <- datawizard::data_tabulate(iris_c$Sepal.Width, iris_c$Species)
  expect_no_error(p <- plot(x))
  expect_s3_class(p, "gg")
})
